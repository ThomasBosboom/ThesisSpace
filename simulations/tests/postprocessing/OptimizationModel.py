...
class Particle:
    def __init__(self, x0, seed=0, **kwargs):
        ...
        self.c1=1        # cognative constant
        self.c2=2        # social constant
...
class OptimizationModel:
    ...
    def __init__(self, json_settings={"save_dict": True, "current_time": float, "file_name": str}, custom_input={}, **kwargs):
        ...
        self.c1=1        # cognative constant
        self.c2=2        # social constant
        self.use_random_initial_design_vector = False
        ...
        self.latest_observation_windows = None
        self.run_counter = 0
        self.num_runs = 1
        ...
        # print("HEEEEEEEEEEEEEEEEEEE", self.use_random_initial_design_vector)
    ...
    def check_new_length(self, observation_windows):

        total_duration = self.duration
        current_duration = observation_windows[-1][-1]-observation_windows[0][0]
        remainder = self.arc_interval + self.arc_length
        if total_duration-current_duration >= remainder:
            return np.inf
        return 0
    ...
    def get_cached_intermediate_results(self, iteration, run_counter):
        history = self.intermediate_iteration_history[str(iteration)][str(run_counter)]
        return history["objective_value"], history["individual_corrections"], history["design_vector"]
    ...
    def particle_swarm(self, objective_function):
        ...
        for i in range(0,self.num_particles):
            ...
            if self.use_random_initial_design_vector:
                if i != 0:
                    x0 = rng.uniform(self.bounds[0], self.bounds[1], len(self.initial_design_vector))
                    # x0 = np.linspace(self.bounds[0], self.bounds[1], len(self.initial_design_vector))
            swarm.append(Particle(x0, seed=seed, c1=self.c1, c2=self.c2))
        ...
        while i < self.total_iterations:
            ...
            for j in range(0,self.num_particles):
                ...
                if self.has_intermediate_iteration_history(i, j):
                    objective_value, individual_corrections, design_vector = self.get_cached_intermediate_results(i, j)
    ...
    def nelder_mead(self, objective_function):
        ...
        def objective(design_vector):
            ...
            if self.has_intermediate_iteration_history(self.iteration, self.run_counter):
                objective_value, individual_corrections, design_vector = self.get_cached_intermediate_results(self.iteration, self.run_counter)
                print(f"Retrieving iteration {self.iteration}, run counter {self.run_counter} from cache....")

            else:
                objective_value, individual_corrections = objective_function(observation_windows)

                # Add constraint on keeping the same number of arcs
                if self.include_constraints:
                    objective_value += self.check_new_length(observation_windows)
                ...
                self.latest_objective_value = objective_value
                self.latest_individual_corrections = individual_corrections
                self.latest_observation_windows = observation_windows
...
#             seed = rng.integers(0, 1000)
#             x0 = self.initial_design_vector
#             swarm.append(Particle(self.initial_design_vector, seed=seed, c1=self.c1, c2=self.c2))

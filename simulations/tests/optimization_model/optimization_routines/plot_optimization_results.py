...
folder_name = "PMSRP01_PMSRP01_thr3_skm3_dur28_od1_num1_max20_test"
batch_name = "09040219"
...
for i in range(len(mean_history[0])):
    ...
    axs[0].errorbar(epochs, mean_history[:, i], yerr=std_history[:, i], fmt='o', capsize=2, color=color_cycle[i], alpha=0.3)
...
for j in range(monte_carlo_stats_dict["num_runs"]):
    ...
    axs[1].errorbar(epochs, mean_history_obj, yerr=std_history_obj, fmt='o', capsize=2, color=color_cycle[i], alpha=0.3)

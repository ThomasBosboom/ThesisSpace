...
#     def __init__(self, mission_start_epoch, mission_time, batch_start_times, batch_end_times, initial_state_error, apriori_covariance):

#         self.mission_start_epoch = mission_end_epoch
...
def perform_navigation(mission_start_epoch, batch_start_times, batch_end_times, initial_state_error, apriori_covariance, estimation_model, truth_model, sigma_number=3):
    ...
    model_type = estimation_model[0]
    model_name = estimation_model[1]
    model_number = estimation_model[2]

    model_type_truth = truth_model[0]
    model_name_truth = truth_model[1]
    model_number_truth = truth_model[2]
    ...
    while mission_epoch < batch_end_times[-1]:

        print(f"Estimation of arc {batch_count}, duration {batch_times[batch_count]} days: {batch_start_times[batch_count]} until {batch_end_times[batch_count]}")
        ...
        if (mission_epoch-mission_start_epoch)%4 == 0 and mission_epoch != mission_start_epoch:

            lists = [[0, [3, 5]]]
            for i, list1 in enumerate(lists):
                dynamic_model.simulation_start_epoch_MJD = batch_start_times[batch_count]
                start_time = time.time()
                station_keeping = StationKeeping.StationKeeping(dynamic_model, custom_initial_state=custom_initial_state, custom_propagation_time=max(list1[1]), step_size=0.01)
    ...
    return full_estimation_error_dict, full_reference_state_deviation_dict, full_propagated_covariance_dict, full_propagated_formal_errors_dict, observation_windows
...
estimation_model = [model_type, model_name, model_number]
truth_model = [model_type_truth, model_name_truth, model_number_truth]

sigma_number = 3



full_estimation_error_dict, full_reference_state_deviation_dict, full_propagated_covariance_dict, full_propagated_formal_errors_dict, observation_windows = perform_navigation(mission_start_epoch, batch_start_times, batch_end_times, initial_state_error, apriori_covariance, estimation_model, truth_model, sigma_number=sigma_number)
...
fig.suptitle(r"Estimation error history: $1\sigma_{\rho}$ = 102.44 [$m^2$], $f_{obs}$ = $1/600$ [$s^{-1}$]")
plt.tight_layout()
plt.show()
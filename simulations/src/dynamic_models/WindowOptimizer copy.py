...
class WindowOptimizer():
    ...
    #     if np.linalg.norm(relative_state_history <
    ...
    def get_observation_windows(self):
        ...
        epochs, state_history, dependent_variables_history, state_transition_history = \
            Interpolator.Interpolator(epoch_in_MJD=False, step_size=self.step_size).get_propagation_results(self.dynamic_model)
        ...
        for key, value in angles_dict.items():
            ...
            above_angle_treshold = value < self.angle_treshold
            filtered_dict[key] = above_angle_treshold
        ...
        # fig = plt.figure()
        # # plt.plot(epochs, relative_state_history[:,:3])
        # # plt.plot(epochs, np.linalg.norm(relative_state_history[:,:3], axis=1), label="abs position")

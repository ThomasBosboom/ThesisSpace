...
class HighFidelityDynamicModel(DynamicModelBase):
    ...
    def __init__(self, simulation_start_epoch_MJD, propagation_time, custom_initial_state=None, custom_ephemeris_settings=False):
        super().__init__(simulation_start_epoch_MJD, propagation_time)

        self.custom_initial_state = custom_initial_state
        self.custom_ephemeris_settings = custom_ephemeris_settings
    ...
    def set_environment_settings(self):
        ...
        if self.custom_ephemeris_settings:
            for index, body in enumerate(self.bodies_to_propagate):

                self.body_settings.get(body).ephemeris_settings = environment_setup.ephemeris.tabulated(
                    validation_LUMIO.get_reference_state_history(self.simulation_start_epoch_MJD,
                ...
                print("I have been here: ", self.custom_ephemeris_settings, body)
    ...
    def set_propagator_settings(self):
        ...
        print("propagator settings reached: ", self.custom_ephemeris_settings)
    ...
    def get_propagated_orbit(self):
        ...
        # fig = plt.figure()
        # plt.plot(np.stack(list(dynamics_simulator.state_history.values()))[:,0], color="red")
        # plt.plot(np.vstack(list(self.body_settings.get(self.name_ELO).ephemeris_settings.body_state_history.values()))[:,0], color="green")
        # plt.show()
...
# setattr(dynamic_model, 'custom_ephemeris_settings', True)

# dynamics_simulator, variational_equations_solver = dynamic_model.get_propagated_orbit()

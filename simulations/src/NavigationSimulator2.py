...
class NavigationSimulator(NavigationSimulatorBase):
    ...
    def perform_navigation(self, observation_windows, seed=0):
        ...
        # tracemalloc.start()
        # Copy attributes to be manipulated later
        apriori_covariance = copy.deepcopy(self.apriori_covariance)
        custom_initial_state = copy.deepcopy(self.custom_initial_state)
        custom_initial_state_truth = copy.deepcopy(self.custom_initial_state_truth)

        # apriori_covariance = self.apriori_covariance
        # custom_initial_state = self.custom_initial_state
        # custom_initial_state_truth = self.custom_initial_state_truth

        # Random noise generator added\
        print("seed: ", seed)
        ...
        initial_station_keeping_epochs = [np.round(observation_window[1], decimal_places) for observation_window in observation_windows]
        ...
        times = list([self.observation_windows[0][0]] + [item for sublist in self.observation_windows for item in sublist] + [self.observation_windows[0][0]] + initial_station_keeping_epochs)
        times = list(set(times))
        times = sorted(times)

        navigation_arc_durations = np.round(np.diff(times), decimal_places)
        estimation_arc_durations = np.round(np.array([tup[1] - tup[0] for tup in self.observation_windows]), decimal_places)
        ...
        for t, time in enumerate(times):
            ...
            navigation_arc_duration = navigation_arc_durations[navigation_arc]
            ...
            if navigation_arc == 0:
                custom_initial_state_truth = truth_model.initial_state + self.orbit_insertion_error
                custom_initial_state = custom_initial_state_truth + self.initial_estimation_error
            ...
            if self.propagate_dynamics_linearly:
                state_history_estimated = state_history_truth + np.dot(state_transition_matrix_history_estimated, custom_initial_state-custom_initial_state_truth)
                # print("length: \n", epochs[0], epochs[-1], len(state_history_estimated))

            # Save the propagated histories of the uncertainties
            if apriori_covariance is not None:
                propagated_covariance_estimated = dict()
                propagated_formal_errors_estimated = dict()
                for i in range(len(epochs)):
                    propagated_covariance = state_transition_matrix_history_estimated[i] @ apriori_covariance @ state_transition_matrix_history_estimated[i].T
            ...
            if time in batch_start_times:
                ...
                estimation_model = EstimationModel.EstimationModel(dynamic_model, truth_model, **estimation_model_kwargs)
            ...
            if estimation_arc_activated:
                ...
                apriori_covariance += self.get_process_noise_matrix(navigation_arc_duration*constants.JULIAN_DAY,
                                                                            self.state_noise_compensation_lpf,
                                                                            self.state_noise_compensation_lumio)

            else:
                custom_initial_state = state_history_estimated[-1,:]
                apriori_covariance = np.stack(list(propagated_covariance_estimated.values()))[-1]
            ...
            if self.include_station_keeping:
                if times[navigation_arc+1] in initial_station_keeping_epochs:
                    ...
                    if self.model_type == "HF":
                        ...
                        if np.linalg.norm(delta_v) >= self.delta_v_min:
                            ...
                            apriori_covariance += np.outer(delta_v_noise_covariance, delta_v_noise_covariance)

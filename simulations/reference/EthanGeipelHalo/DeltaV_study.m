...
m1 = 5.973698990946545*10^24; m2 = 7.347671776396872*10^22; % Actual Masses (earth-moon)
G = (6.67259e-11); mu = m2/(m1+m2);
au = 3.84747963e5; 
...
xlim([min(Y_final(:,1))*.9975 max(Y_final(:,1))*1.0025]); ylim([-max(Y_final(:,2))*1.1 max(Y_final(:,2))*1.1])

subplot(1,3,2)
plot(L2(1)+Y_final(:,2), Y_final(:,3), 'k', 'LineWidth', 1.5); hold on
plot(L2,0,'kx')
xlabel('Y'); ylabel('Z'); axis square; box off; title('Y-Z Plot')
xlim([min(L2(1)+Y_final(:,2))*.9995 max(L2(1)+Y_final(:,2))*1.0005]); ylim([min(Y_final(:,3))*1.05 max(Y_final(:,3))*1.05])

subplot(1,3,3)
plot(Y_final(:,1), Y_final(:,3), 'k', 'LineWidth', 1.5); hold on
plot(L2,0,'kx')
xlabel('X'); ylabel('Z'); axis square; box off; title('X-Z Plot')
xlim([min(Y_final(:,1))*.9975 max(Y_final(:,1))*1.0025]); ylim([min(Y_final(:,3))*1.05 max(Y_final(:,3))*1.05])
...
for i = 2:1:length(Y_xyz)
    ...
    if( delay < 1)
        ...
        [a, b] = min(displaces);
        [deltaV, Y_prop] = StationKeep(Y_delta(i,1:6)', Y_delta(i,7:end)', propOut, t_0, h, i, mu, 1, STMsave, Y_xyz, b);
        delay = 50;
        
        % Checks if conditions for making a manuever have been met (minimum
        % delta V threshold, 2 m/s max, etc.)
        if ((abs(norm(deltaV)*au*ne*1000) > 1e-1 && abs(norm(deltaV)*au*ne*1000) < 2) || (quickFix == 1) ) 
            burnCounter(end+1) = i;
            delay = 1000;
...
hold on; plot3(au*L2, 0, 0); title('Nominal'); axis square
...
plot3(Y_delta_nc(:,1), Y_delta_nc(:,2), Y_delta_nc(:,3), 'Color', [0.9290 0.6940 0.1250], 'LineWidth', 0.9)
hold on; plot3(au*L2, 0, 0); title('Not Corrected'); axis square
xlim([xlm-xlw xlm+xlw]); ylim([ylm-ylw ylm+ylw]); zlim([zlm-zlw zlm+zlw]);
set(gca,'YTickLabel',[]); xlabel('X')
set(gca,'XTickLabel',[]); ylabel('Y')
set(gca,'ZTickLabel',[]); zlabel('Z')
...
plot3(Y_xyz(:,1), Y_xyz(:,2), Y_xyz(:,3), 'Color', [0.4660 0.6740 0.1880], 'LineWidth', 0.9)
hold on; plot3(au*L2, 0, 0); title('Nominal')
xlim([xlm-xlw xlm+xlw]); ylim([ylm-ylw ylm+ylw]); zlim([zlm-zlw zlm+zlw]);
set(gca,'YTickLabel',[]); xlabel('X')
set(gca,'XTickLabel',[]); ylabel('Y')
set(gca,'ZTickLabel',[]); zlabel('Z')
...
plot3(Y_delta(:,1), Y_delta(:,2), Y_delta(:,3), 'k', 'LineWidth', 0.9)
hold on; plot3(au*L2, 0, 0); title('Corrected'); hold on;
xlim([xlm-xlw xlm+xlw]); ylim([ylm-ylw ylm+ylw]); zlim([zlm-zlw zlm+zlw]);

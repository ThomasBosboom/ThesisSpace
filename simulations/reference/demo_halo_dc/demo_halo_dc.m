...
for i = 2:1:npts
    ...
    vxplot(i) = dist/(tu) * ysol(length(twrk), 4);
    vyplot(i) = dist/(tu) * ysol(length(twrk), 5);
    vzplot(i) = dist/(tu) * ysol(length(twrk), 6);
...
for i = 1:length(xplot)
    ...
    k = k + 1;
    x_crtbp_dc(k,:) = [xplot(i)/dist, yplot(i)/dist, zplot(i)/dist, vxplot(i)*tu/dist, vyplot(i)*tu/dist, vzplot(i)*tu/dist];

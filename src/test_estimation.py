...
integrator_settings = propagation_setup.integrator.\
    runge_kutta_fixed_step_size(initial_time_step=30.0,
...
dependent_variables_to_save = [propagation_setup.dependent_variable.relative_position("Delfi-C3", "Earth"), 
                               propagation_setup.dependent_variable.relative_velocity("Delfi-C3", "Earth")]
...
print(observation, link_definition)
...
viability_setting_list = [observation.elevation_angle_viability(["Earth", "TrackingStation"], np.deg2rad(15)),
                    #  observation.body_occultation_viability(["Earth", "TrackingStation"], "Earth")
                     ]
...
# print("variational solver state history: ", estimator.variational_solver.state_history)
...
print("simulated_observations type: ", simulated_observations)
single_observation_set_list = simulated_observations.get_single_link_and_type_observations(observation.one_way_instantaneous_doppler_type, link_definition)


for single_observation_set in single_observation_set_list:
    observations = single_observation_set.concatenated_observations

print(observations)
ax = plt.figure()
plt.plot(observations, color="red", label="LPF")
plt.show()
...
print("covariance_input type: ", covariance_input)

# Set methodological options
covariance_input.define_covariance_settings(
    reintegrate_variational_equations=False, save_design_matrix=True)
...
print(covariance_output.design_matrix)
# print(covariance_output.normalized_design_matrix)
